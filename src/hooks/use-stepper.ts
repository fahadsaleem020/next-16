  ...
  switch (type) {
    ...
    case "setSteps": {
      if (Array.isArray(step)) {
        const newSteps = Object.fromEntries(step.map((key) => [key, { isActive: false, isFinished: false }]));
...
export const useStepper = <T extends string>(initialSteps: T[], initialStep?: T) => {
  const initialData: State = Object.fromEntries(initialSteps.map((key) => [key, { isActive: false, isFinished: false }]));
